...
public:
    ...
    bool write(TTime now, TValue val) {
        ...
        if (_last_cursor == cursor) {
            ...
            _history[cursor].min = NYNA_MIN(_history[cursor].min, val);
            _history[cursor].max = NYNA_MAX(_history[cursor].max, val);
            _history[cursor].ave = NYNA_ROUND_DIV(_accum_value, _accum_count);
        }
        else {
            _history[cursor].valid = true;
    ...
    void get_min_max(TValue *min_val, TValue *max_val, TValue default_value = DEFAULT_VAL, bool use_ave = false) const {
        ...
        for (int i = 0; i < HISTORY_SIZE; i++) {
            ...
            if (i == 0) {
                ...
                min_hold = NYNA_MIN(min_hold, this_min);
                max_hold = NYNA_MAX(max_hold, this_max);
            }
        }
    ...
    TValue total_average(TValue default_value = DEFAULT_VAL) const { 
        ...
        return NYNA_ROUND_DIV(accum, _size);
    }
